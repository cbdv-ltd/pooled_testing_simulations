...
# for tomorrow, the 5 at 2,0 isn't found, (prematurely eliminated?) check the elimination layer for when it goes. there is a problem in itteration 3.
def guess(matrix, fail: float, tests = 0, ool = 0):
    ...
    while count < dim * dim + 2 and elim > 0:        
        ...
        # print((x_av > x_fail) * m[:,:,2])
        # print((y_av > y_fail) * m[:,:,2])
        # print("-----------------------------\n",(x_av > x_fail) * m[:,:,2]*(y_av > y_fail),"\n-------------------------\n")
        m[:,:,2] = (x_av > x_fail) * m[:,:,2]*(y_av > y_fail)
        # print(x_av, "\n", y_av)
